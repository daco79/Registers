...
DB_USER="${SSH_USER}_${CPANEL_DB_USER_SHORT}"
DB_PASS="${REGISTERS_DB_PASS:-}"

if [ -z "$DB_PASS" ]; then
  echo "Erreur : definis REGISTERS_DB_PASS avant de lancer ce script." >&2
  echo "Exemple : REGISTERS_DB_PASS='...' ./scripts/o2-init-db.sh" >&2
  exit 1
fi
...
ssh -i "$SSH_KEY" "${SSH_USER}@${SSH_HOST}" \
  "UAPI_DB_NAME='$UAPI_DB_NAME' DB_NAME='$DB_NAME' DB_USER='$DB_USER' DB_PASS='$DB_PASS' REMOTE_DIR='$REMOTE_DIR' bash -s" <<'ENDSSH'
set -euo pipefail
